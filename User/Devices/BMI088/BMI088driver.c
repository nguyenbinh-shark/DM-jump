 ...
 ******************************************************************************
 * @file    BMI088driver.c
 * @brief   Driver hiện thực giao tiếp, khởi tạo và hiệu chuẩn cảm biến IMU BMI088
...
/* Bảng thanh ghi cấu hình cảm biến gia tốc (Accelerometer) */
...
/* Bảng thanh ghi cấu hình con quay hồi chuyển (Gyroscope) */
...
/**
 * @brief  Khởi tạo cảm biến BMI088 (lặp cho đến khi thành công)
 * @param  bmi088_SPI: Con trỏ tới SPI handle của STM32
 * @param  calibrate:  Cờ cho phép tự động hiệu chuẩn (1: hiệu chuẩn tĩnh, 0: nạp offset mặc định)
 */
...
/**
 * @brief  Cấu hình và kiểm tra kết nối với cả hai khối Accel và Gyro của BMI088
 * @param  bmi088_SPI: Con trỏ tới SPI handle
 * @param  calibrate:  1 để hiệu chuẩn động khi đứng yên
 * @retval Mã lỗi (BMI088_NO_ERROR nếu thành công)
 */
uint8_t BMI088_init(SPI_HandleTypeDef *bmi088_SPI, uint8_t calibrate)
{
    ...
    if (calibrate)
    {
        /* Thực hiện chu trình lấy mẫu để tính toán offset tĩnh cho con quay và gia tốc kế */
        Calibrate_MPU_Offset(&BMI088);
    }      
    else
    {
        /* Sử dụng giá trị offset mặc định đã được căn chỉnh trước */
...
/**
 * @brief  Thuật toán tự động hiệu chuẩn điểm 0 (Zero-rate Bias Calibration)
 * @note   Yêu cầu robot phải đứng yên hoàn toàn trên mặt phẳng trong suốt quá trình này
 * @param  bmi088: Con trỏ cấu trúc dữ liệu IMU
 */
void Calibrate_MPU_Offset(IMU_Data_t *bmi088)
{
    static float startTime;
    static uint16_t CaliTimes = 6000;       /* Số mẫu đo con quay hồi chuyển */
    static uint16_t acc_CaliTimes = 6000;   /* Số mẫu đo gia tốc kế */
    ...
    do
    {
        /* Nếu thời gian hiệu chuẩn vượt quá 10 giây (robot bị rung/chuyển động), nạp offset mặc định */
        ...
        for (uint16_t i = 0; i < CaliTimes; i++)
        {
            ...
            /* Theo dõi dải dao động cực đại - cực tiểu để phát hiện rung lắc */
            if (i == 0)
            {
                gNormMax = gNormTemp;
                gNormMin = gNormTemp;
            ...
            /* Nếu biên độ dao động vượt ngưỡng cho phép, huỷ lần lặp này */
        ...
        /* Tính trung bình cộng độ lệch tĩnh */
        bmi088->gNorm /= (float)CaliTimes;
        for (uint8_t i = 0; i < 3; i++)
            bmi088->GyroOffset[i] /= (float)CaliTimes;

        /* Đọc nhiệt độ IMU tại thời điểm hiệu chuẩn */
    ...
    /* Tính hệ số co dãn gia tốc chuẩn 9.81 m/s^2 */
    bmi088->AccelScale = 9.81f / bmi088->gNorm;
            
    /* Lọc thông thấp offset gia tốc kế */
...
/**
 * @brief  Khởi tạo phần cứng khối gia tốc kế BMI088 (Accelerometer)
 * @retval Mã lỗi khởi tạo
 */
uint8_t bmi088_accel_init(void)
{
    ...
    /* Reset mềm (Software Reset) khối gia tốc kế */
    BMI088_accel_write_single_reg(BMI088_ACC_SOFTRESET, BMI088_ACC_SOFTRESET_VALUE);
    HAL_Delay(BMI088_LONG_DELAY_TIME);

    /* Đọc kiểm tra sau reset */
    BMI088_accel_read_single_reg(BMI088_ACC_CHIP_ID, res);
    HAL_Delay(1);
    BMI088_accel_read_single_reg(BMI088_ACC_CHIP_ID, res);
    HAL_Delay(1);

    /* Xác thực Chip ID của Accelerometer (0x1E) */
    if (res != BMI088_ACC_CHIP_ID_VALUE)
        return BMI088_NO_SENSOR;

    /* Ghi tuần tự cấu hình thanh ghi và kiểm tra tính toàn vẹn */
...
/**
 * @brief  Khởi tạo phần cứng khối con quay hồi chuyển BMI088 (Gyroscope)
 * @retval Mã lỗi khởi tạo
 */
uint8_t bmi088_gyro_init(void)
{
    ...
    /* Xác thực Chip ID Gyroscope (0x0F) */
    if (res != BMI088_GYRO_CHIP_ID_VALUE)
        return BMI088_NO_SENSOR;

    /* Ghi cấu hình dải đo và tần số lấy mẫu Gyroscope */
...
/**
 * @brief  Đọc toàn bộ dữ liệu đo mới nhất từ cảm biến BMI088 (Gia tốc, Tốc độ góc, Nhiệt độ)
 * @param  bmi088: Con trỏ cấu trúc dữ liệu đích
 */
void BMI088_Read(IMU_Data_t *bmi088)
{
    ...
    if (caliOffset)
    {
        ...
        bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];
        bmi088->Accel[2] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale; /* Không trừ offset trục Z vì có trọng trường g */
    }
    else
    {
        bmi088_raw_temp = (int16_t)((buf[1]) << 8) | buf[0];
    ...
    /* 2. Đọc dữ liệu Con quay hồi chuyển 3 trục */
    BMI088_gyro_read_muli_reg(BMI088_GYRO_CHIP_ID, buf, 8);
    if (buf[0] == BMI088_GYRO_CHIP_ID_VALUE)
    {
        if (caliOffset)
