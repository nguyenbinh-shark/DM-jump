...
void FDCAN1_Config(void)
{
  ...
  sFilterConfig.FilterID1 = 0x00000000; // Mask ID1
  sFilterConfig.FilterID2 = 0x00000000; // Mask ID2
...
/* Global filter:
 * Reject non-matching STD IDs
 * Reject non-matching EXT IDs
 * Reject STD remote frames
 * Reject EXT remote frames
 */
...
uint8_t canx_send_data(FDCAN_HandleTypeDef *hcan, uint16_t id, uint8_t *data, uint32_t len)
{
  ...
  if(len<=8)	
	{
     TxHeader.DataLength = len<<16;     // Data length (<=8 bytes)
	}
	else  if(len==12)	
	{
  ...
  TxHeader.FDFormat =  FDCAN_CLASSIC_CAN; // Classic CAN (no FD payload)
...
{ 
  if((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
  {
    if(hfdcan->Instance == FDCAN1)
    {
      ...
      memset(g_Can1RxData, 0, sizeof(g_Can1RxData));	// Clear RX buffer before read	
